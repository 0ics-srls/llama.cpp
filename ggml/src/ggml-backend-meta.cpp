...
void ggml_backend_meta_alloc_ctx_tensors_from_buft_size_per_dev(struct ggml_context * ctx, ggml_backend_buffer_type_t buft, size_t * sizes) {
    // volta-ada: stessa costruzione di ggml_backend_meta_alloc_ctx_tensors_from_buft, ma senza allocare: per ogni scheda
    // semplice la dimensione delle sue fette (tipo per scheda e split inclusi). I tensori di ctx vengono ripristinati.
    const size_t n_simple_bufts = ggml_backend_meta_buft_n_bufts(buft);
    ...
    struct saved_t { ggml_tensor * t; ggml_backend_buffer_t buffer; void * data; };
    std::vector<saved_t> saved;
    for (ggml_tensor * t = ggml_get_first_tensor(ctx); t != nullptr; t = ggml_get_next_tensor(ctx, t)) {
        saved.push_back({t, t->buffer, t->data});
        t->buffer = meta_buf;
        ggml_backend_meta_buffer_init_tensor_impl(meta_buf_ctx->stc_static, t);
    }
    for (size_t i = 0; i < n_simple_bufts; i++) {
        ggml_context * ctx_i = meta_buf_ctx->stc_static.ctxs[i].get();
        sizes[i] = ggml_backend_alloc_ctx_tensors_from_buft_size(ctx_i, ggml_backend_meta_buft_simple_buft(buft, i));
    }
    for (const saved_t & s : saved) {
        s.t->buffer = s.buffer;
        s.t->data   = s.data;
    }
